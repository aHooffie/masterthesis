...
LUT addDefs(CRDSII c, LUT lut)
{
	visit(c) {
		...
		case token(ID name, _, _, _, _): 				{ lut = addDef(name.name, name@location, "token", lut);}
	}
...
// case card(_, list[Exp] exps):								{ lut = addAttrRefs(exps, lut); }							// TO DO: Attr
// case points(list[Scoring] scores):							{ lut = addScores(scores, lut); }							// TO DO: Attr
// case obj(ID name, ID attr):									{ lut = checkRef(name, lut); lut = checkRef(attr, lut); }	// TO DO: Attr
