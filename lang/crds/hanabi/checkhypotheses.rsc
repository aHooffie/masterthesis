...
data Decks 
 = decks(map[str name, list[str] cards] cardsets,
 		 map[str name, str oldest] deckhistory,
		...
		visit(ast) {
			case deck(ID name, list[Card] cards, _, list[Prop] props, list[Condition] cdns):
																		{ as.ds.cardsets += (name.name : getCards(cards));
																		  ...
																		  int bla = size(as.ds.cardsets[name.name]);
																		  if (bla != 0)
																		      as.ds.deckhistory[name.name] = as.ds.cardsets[name.name][bla - 1];} 
...
Administration runAction(communicate(list[ID] locations, Exp e), Administration as, str currentPlayer, list[str] currentTurn) {	
	...
	if (cat == 5 && as.ds.deckhistory == target && "At least one playing convention has been introduced by the players." notin as.ch.booleans) { // && Oldest card in hand)
		// "A player gave another player more options to execute in the turn" notin as.ch.booleans)
		println("\n\n\n\n FOUND IT");
		as.ch.booleans += ["At least one playing convention has been introduced by the players."];
	}
...
Administration runAction( req(distributeCards(real r, ID from, list[ID] locations)), Administration as) {	
	...
	list[str] targets = [ location.name | location <- locations]; // hands
	...
	for (int i <- [0 .. toInt(r)]) {
	 	for (int j <- [0 .. size(targets)]) {
			...
			as.ds.deckhistory[targets[j]] = (name.name : result.card);
		}
	}
	
	println(as.ds.deckhistory);
...
Administration runAction(choice(real r, list[Action] actions), Administration as, str currentPlayer, list[str] currentTurn) {	
	...
	if (currentPlayer in as.ph.memory) {	
		...
		if (size(possiblePlays) > 1 && "A player knows he can play multiple cards." notin as.ch.booleans)
...
Administration runAction(moveCard(Exp e, list[ID] fromList, list[ID] to), Administration as, str currentPlayer, list[str] currentTurn) {	
	...
	if (movedCard in as.ph.memory[currentPlayer] && "A player successfully collaborated by reducing another player\'s options." notin as.ch.booleans)
		as.ch.booleans += ["A player successfully collaborated by reducing another player\'s options."];	
	...
	// Check if card can be played. 
		...
		if (size(as.ds.cardsets[correctPile]) > 1 && "Player successfully worked together towards the same goal." notin as.ch.booleans)
			as.ch.booleans += ["Player successfully worked together towards the same goal."];
