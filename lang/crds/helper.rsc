...
public list[str ] getCards(list[Card] cards) {
	list[str ] allCards = [];
	for (card <- cards) {
		if (card(var(ID n), list[Exp] attrs) := card) allCards += n.name; 
	}
...
public bool checkPlay(distributeCards(real r, ID name, list[ID] locations), Tokens ts, Decks ds) {
	if (ds.cardsets[name.name] >= size(locations) * r) return true;
	return false;
}
